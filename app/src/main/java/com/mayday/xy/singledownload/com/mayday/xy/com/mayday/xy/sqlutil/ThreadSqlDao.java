...
public class ThreadSqlDao {
     ...
     * @param url
     */
    public void deleData(int Threadid,String url){
        DeleteBuilder builder = dao.deleteBuilder();
        try {
            //delete from ThreadInfo where id=? and url=?;
            builder.where().eq("id",Threadid).and().eq("url",url);
            builder.delete();
        } catch (SQLException e) {
            e.printStackTrace();
        }
     ...
     * @throws SQLException
     */
    public void updatable(int Threadid,String url,int finished) throws SQLException {
